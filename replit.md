...
### Novembro 2025 - Filtros de Categoria no Painel do Garçom

**Sistema de Filtros de Produtos:**
- Adicionados filtros de categoria no painel do garçom (mesa individual)
- Botões de filtro similares ao cardápio público para melhor UX
- Funcionalidade JavaScript para filtrar produtos por categoria em tempo real
- Botão "Todos" para exibir todos os produtos de todas as categorias
- Melhor organização visual dos produtos para facilitar a busca pelo garçom

### Novembro 2025 - Sistema de Perfis e Fluxo de Pagamento Segregado

**Sistema de Login por Perfil:**
- Implementadas páginas de login específicas para cada perfil:
  - `/auth/login/garcom` - Acesso exclusivo para garçons
  - `/auth/login/caixa` - Acesso exclusivo para caixa
  - `/auth/login/cozinha` - Acesso exclusivo para cozinha
- Cada página possui ícone e título personalizados
- Validação de role no backend impede acesso não autorizado
- Redirecionamento automático para área específica após login

**Novo Fluxo de Pagamento (Garçom → Caixa):**
- Garçom solicita fechamento da mesa, mas NÃO libera a mesa
- Pedido fica com status `aguardando_pagamento`
- Mesa permanece ocupada até confirmação do caixa
- Caixa visualiza pedidos pendentes em destaque
- Apenas o CAIXA pode confirmar pagamentos e liberar mesas
- Proteção de segurança: endpoints de caixa validam role antes de executar

**Melhorias Visuais:**
- Produtos na página do garçom agora exibem imagens
- Fallback com ícone quando produto não possui imagem
- Layout responsivo com cards uniformes

**Usuários de Teste Criados:**
- `garcom1` / senha: `garcom123` - Acesso ao PDV/Mesas
- `caixa1` / senha: `caixa123` - Acesso ao Caixa
- `cozinha1` / senha: `cozinha123` - Acesso ao KDS/Cozinha
- `admin` / senha: `admin123` - Acesso total ao sistema
