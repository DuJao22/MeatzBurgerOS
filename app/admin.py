...
from app.db_operations_extended import (ProductModifierGroup, ProductModifierOption, ServiceChargePolicy, 
                                        ...
                                        CashierSession, AuditLog, PromotionalBanner)
...
def api_metrics():
    ...
    for order in active_orders:
        ...
        kitchen_orders_data.append({
            'id': order['id'],
            'table_number': dict(order).get('table_number', 'N/A'),
            'status': order['status'],
            'age_minutes': int(order_age_minutes),
    ...
    if request.method == 'POST':
        ...
        if 'image_file' in request.files:
            ...
            if file and file.filename:
                ...
                upload_path = os.path.join('app', 'static', 'uploads', 'products', filename)
...
def edit_product(id):
    ...
    if request.method == 'POST':
        price_str = request.form.get('price')
        image_url = request.form.get('image_url', product['image_url'] or '')
    ...
    if request.method == 'POST':
        ...
        if store_zipcode:
            ...
            if geo_data:
                ...
                flash('CEP inválido ou não encontrado! Por Favor coloque um CEP Valido!', 'warning')
...
@admin_required
def banners():
    banners = PromotionalBanner.get_all()
    return render_template('admin/banners.html', banners=banners)

@admin.route('/banner/adicionar', methods=['GET', 'POST'])
@login_required
@admin_required
def add_banner():
    if request.method == 'POST':
        ...
        if 'image_file' in request.files:
            ...
            if file and file.filename:
                ...
                timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
                filename = f"{timestamp}_{filename}"
                upload_path = os.path.join('app', 'static', 'uploads', 'banners', filename)
                file.save(upload_path)
                image_url = f"/static/uploads/banners/{filename}"
        
        if not title or not image_url:
            flash('Título e imagem são obrigatórios', 'danger')
...
@admin.route('/banner/<int:id>/editar', methods=['GET', 'POST'])
@login_required
@admin_required
def edit_banner(id):
    banner = PromotionalBanner.get_by_id(id)
    if not banner:
        abort(404)
    ...
    if request.method == 'POST':
        ...
        image_url = request.form.get('image_url', banner['image_url'])
...
@admin.route('/banner/<int:id>/deletar', methods=['POST'])
@login_required
@admin_required
def delete_banner(id):
    PromotionalBanner.delete(id)
    flash('Banner deletado com sucesso!', 'success')
    return redirect(url_for('admin.banners'))

@admin.route('/banner/<int:id>/toggle', methods=['POST'])
@login_required
@admin_required
def toggle_banner(id):
    PromotionalBanner.toggle_active(id)
    flash('Status do banner atualizado!', 'success')
    return redirect(url_for('admin.banners'))

# Rota para gerar QR codes para mesas
@admin.route('/mesas/qrcodes')
@login_required
@admin_required
def table_qrcodes():
    tables = Table.get_all()
    return render_template('admin/table_qrcodes.html', tables=tables)

@admin.route('/mesa/<int:id>/qrcode')
@login_required
@admin_required
def generate_table_qrcode(id):
    table = Table.get_by_id(id)
    if not table:
        abort(404)
    
    # Gera URL do cardápio digital para a mesa
    menu_url = url_for('main.digital_menu', table_id=id, _external=True)
