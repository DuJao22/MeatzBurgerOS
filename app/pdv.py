...
def table_order(table_id):
    ...
    if table['current_order_id']:
        ...
        if current_order:
            order_items = OrderItem.get_by_order(current_order['id'])
...
def finalize_order(order_id):
    ...
    if request.method == 'POST':
        ...
        Order.update(order_id, status='aguardando_pagamento', payment_method=payment_method)
        
        flash('Pedido enviado para o caixa confirmar o pagamento!', 'success')
        return redirect(url_for('pdv.index'))
    
    order_items = OrderItem.get_by_order(order['id'])
    return render_template('pdv/finalize_order.html', order=order, order_items=order_items)
...
def cashier():
    ...
    pending_orders = Order.get_by_status('aguardando_pagamento')
...
@pdv.route('/caixa/confirmar-pagamento/<int:order_id>', methods=['POST'])
@login_required
def confirm_payment(order_id):
    if current_user.role not in ['caixa', 'admin']:
        return jsonify({'success': False, 'error': 'Acesso negado. Apenas o caixa pode confirmar pagamentos.'}), 403
    ...
    if order_dict['status'] != 'aguardando_pagamento':
        return jsonify({'success': False, 'error': 'Pedido não está aguardando pagamento'}), 400
    
    payments = Payment.get_by_order(order_dict['id'])
    for payment in payments:
        if payment['status'] == 'pendente':
            Payment.update(payment['id'], status='pago')
    
    Order.update(order_id, status='pago')
    
    if order_dict.get('table_id'):
        Table.update(order_dict['table_id'], status='livre', current_order_id=None)
    
    flash(f'Pagamento confirmado! Mesa liberada.', 'success')
    return jsonify({'success': True})
...
def kds():
    ...
    sections = list(set([dict(p).get('prep_section', 'geral') for p in all_products if dict(p).get('prep_section')]))
    ...
    for order in pending_orders:
        ...
        for item in items:
            ...
            items_with_section.append({
                ...
                'quantity': item['quantity'],
                'status': item_dict.get('status', 'novo'),
                'notes': item_dict.get('notes', ''),
                'prep_section': product_dict.get('prep_section', 'geral')
            })
        
        order_dict = dict(order)
        order_data = {
            'id': order['id'],
            'table_number': order_dict.get('table_number', 'N/A'),
            'status': order['status'],
            'age_minutes': int(order_age_minutes),
            'priority_class': priority_class,
...
def split_bill(table_id):
    ...
    total = order['total']
    order_dict = dict(order)
    tip = order_dict.get('tip_amount', 0) if order_dict.get('tip_amount') else 0
