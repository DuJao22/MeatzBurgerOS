...
from app.db_operations_extended import PromotionalBanner
...
def index():
    ...
    banners = PromotionalBanner.get_active()
    return render_template('index.html', categories=categories, products=featured_products, banners=banners)
...
def menu():
    ...
    banners = PromotionalBanner.get_active()
    return render_template('menu.html', categories=categories, products=products, banners=banners)

@main.route('/cardapio/digital')
@main.route('/cardapio/digital/<int:table_id>')
def digital_menu(table_id=None):
    categories = Category.get_all()
    products = Product.get_all(available_only=True)
    banners = PromotionalBanner.get_active()
    table = None
    ...
    return render_template('digital_menu.html', categories=categories, products=products, banners=banners, table=table)
...
def track_order(order_id):
    ...
    if order['status'] == 'pendente':
        ...
        estimated_time = 'Chegando em breve'
...
@main.route('/pedido/<int:order_id>/confirmar-entrega', methods=['POST'])
def confirm_delivery(order_id):
    ...
    # Apenas permitir confirmação se estiver em 'saiu_para_entrega'
    if order['status'] == 'saiu_para_entrega':
        Order.update(order_id, status='entregue')
        flash('Obrigado por confirmar o recebimento! Bom apetite!', 'success')
    else:
        flash('Este pedido não pode ser confirmado no momento.', 'warning')
