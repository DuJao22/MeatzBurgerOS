...
class PromotionalBanner:
    @staticmethod
    def create(title, image_url, description=None, link_url=None, active=True, display_order=0):
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO promotional_banner (title, description, image_url, link_url, active, display_order)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (title, description, image_url, link_url, 1 if active else 0, display_order))
            return cursor.lastrowid
    ...
    def get_all():
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM promotional_banner ORDER BY display_order ASC, created_at DESC')
            return cursor.fetchall()
    ...
    def get_active():
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM promotional_banner WHERE active = 1 ORDER BY display_order ASC, created_at DESC')
            return cursor.fetchall()
    ...
    def get_by_id(banner_id):
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM promotional_banner WHERE id = ?', (banner_id,))
            return cursor.fetchone()
    
    @staticmethod
    def update(banner_id, title, image_url, description=None, link_url=None, active=True, display_order=0):
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                UPDATE promotional_banner 
                SET title = ?, description = ?, image_url = ?, link_url = ?, active = ?, display_order = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            ''', (title, description, image_url, link_url, 1 if active else 0, display_order, banner_id))
            return cursor.rowcount > 0
    ...
    def delete(banner_id):
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('DELETE FROM promotional_banner WHERE id = ?', (banner_id,))
            return cursor.rowcount > 0
    ...
    def toggle_active(banner_id):
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('UPDATE promotional_banner SET active = NOT active, updated_at = CURRENT_TIMESTAMP WHERE id = ?', (banner_id,))
            return cursor.rowcount > 0
